| 2026-09-16 | **buildings UX ↔ #297** | #296 consumes `buildingDescModel` / `buildingWalletModel` / `buildingArtSrc` (no parallel models). DOM: overview + upgrade sheet. Draft only. **v1.18.168 / SW v378**. |
| 2026-09-16 | **buildings UX harden** | List→detail, Oogsten één tap, leesbare #buildingsWallet chips, “wat doet dit?” via buildingTooltipModel. Upgrade aparte stap. Draft only, geen Versus. **v1.18.168 / SW v378**. |
