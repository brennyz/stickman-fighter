...
must(/hub-tile[\s\S]*id="sfTitleStart"/.test(index) || /id="sfTitleStart"[^>]*hub-tile/.test(index), 'SPELEN must use hub-tile chrome');
must(!/id="sfTitleName"/.test(index), 'title name field should stay off the start gate');
must(!/sf-title-name-wrap/.test(index), 'title name wrap should be gone');
