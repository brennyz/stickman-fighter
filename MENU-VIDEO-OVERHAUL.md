...
**Status:** plan (nog geen implementatie)  
...
### Fase 0 — Plan & tokens (deze PR)

- [x] Wireframe-zones documenteren
- [ ] Link vanuit `AGENTS.md` / `IMPROVEMENT.md`
- [ ] Geen gameplay-wijziging

### Fase 1 — Layout shell (CSS/HTML, canvas blijft bron)

- Full-bleed `.menu-stage` achter chrome (canvas uitgerekt `object-fit: cover`)
- Titel-glass (B), tile-glass (C), utility-rij D+E
- Verwijder inset “arcade-stage kaart”-look op landing
- First viewport: **merk + korte regel + game-grid + utility** — geen stats-strips / extra promo’s
- Behoud touch-scroll contract (`#menuScreen` als scroll surface op Android)
